...
declare namespace Redux {
    /**
     * An *actions.tsx* is a plain object that represents an intention to change the
     * state. Actions are the only way to get data into the store. Any data,
     * whether from UI events, network callbacks, or other sources such as
     * WebSockets needs to eventually be dispatched as actions.
     *
     * Actions must have a `type` field that indicates the type of actions.tsx being
     * performed. Types can be defined as constants and imported from another
     * module. It’s better to use strings for `type` than Symbols because strings
     * are serializable.
     *
     * Other than `type`, the structure of an actions.tsx object is really up to you.
     ...
     * state and an actions.tsx. They must be *pure functions*—functions that return
     ...
     *   return undefined for any actions.tsx. Instead, they should return their
     *   initial state if the state passed to them was undefined, and the current
     *   state for any unrecognized actions.tsx.
     *
     ...
     * accepts an actions.tsx or an async actions.tsx; it then may or may not dispatch one
     * or more actions to the store.
     *
     * We must distinguish between dispatching functions in general and the base
     * `dispatch` function provided by the store instance without any middleware.
     *
     * The base dispatch function *always* synchronously sends an actions.tsx to the
    ...
    interface Store<S> {
      ...
      /**
       * Adds a change listener. It will be called any time an actions.tsx is
     ...
     *   current state tree and the actions.tsx to handle.
     *
    ...
    /* actions.tsx creators */

    /**
     * An *actions.tsx creator* is, quite simply, a function that creates an actions.tsx. Do
     * not confuse the two terms—again, an actions.tsx is a payload of information, and
     * an actions.tsx creator is a factory that creates an actions.tsx.
     *
     * Calling an actions.tsx creator only produces an actions.tsx, but does not dispatch
     * it. You need to call the store’s `dispatch` function to actually cause the
     * mutation. Sometimes we say *bound actions.tsx creators* to mean functions that
     * call an actions.tsx creator and immediately dispatch its result to a specific
     * store instance.
     *
     * If an actions.tsx creator needs to read the current state, perform an API call,
     * or cause a side effect, like a routing transition, it should return an
     * async actions.tsx instead of an actions.tsx.
     *
     * @template A Returned actions.tsx type.
     */
    ...
    /**
     * Object whose values are actions.tsx creator functions.
     */
    ...
    /**
     * Turns an object whose values are actions.tsx creators, into an object with the
     ...
     *
     * @param actionCreator An object whose values are actions.tsx creator functions.
     ...
     *
     * @returns The object mimicking the original object, but with every actions.tsx
